...
class AgentManager:
    ...
    def resolve_agent(self, agent_id: int) -> DefaultAgent:
        ...
        logger.info(
            "[DEBUG] resolve_agent DB lookup agent_id=%s found=%s",
            agent_id,
            bool(agent_model),
        )
        if not agent_model:
        ...
        logger.info(
            "[DEBUG] resolve_agent created fresh instance agent_id=%s name=%s",
            agent_id,
            agent_model.name,
        )
    ...
    def build_prompt_for_agent(
        ...
        Args:
            agent_id: The agent whose prompt template to use
            messages: The conversation messages
            execution_context: Optional context for dynamic prompt variables
        """
        logger.info(
            "[DEBUG] build_prompt_for_agent called agent_id=%s input_messages_count=%d",
            agent_id,
            len(messages),
        )
        agent = self.resolve_agent(agent_id)
        result = agent.buildPrompt(messages, execution_context=execution_context)
        logger.info(
            "[DEBUG] build_prompt_for_agent result messages_count=%d", len(result)
        )
        for i, msg in enumerate(result):
            logger.info(
                "[DEBUG] build_prompt_for_agent msg[%d] role=%s content_preview=%s",
                i,
                msg.get("role"),
    ...
    def get_agent_config(self, agent_id: int) -> Dict[str, Any]:
        ...
        if agent.agent_model.preferred_model_id:
            preferred_model = (
                self.db.query(Model)
                .filter(Model.id == agent.agent_model.preferred_model_id)
                .first()
            )
        config = {
        ...
        # Warn loudly when the agent has no provider configured so callers
        # are aware they MUST supply a request override; otherwise execution
        # will fail with "Provider with ID None does not exist."
        if config["provider_id"] is None:
            logger.warning(
                "[AGENT_CONFIG] Agent has no provider_id configured — "
                "agent_id=%s name=%s model=%s provider_id=%s. "
                "A request-level provider_id override is required to "
                "execute this agent.",
                agent_id,
    ...
    def validate_execution(self, provider_id: int, model: str):
        ...
        if not model_record:
            raise ValueError(
                f"Model {model} is not available for provider {provider.name}."
            )
